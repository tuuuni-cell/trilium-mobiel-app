...
> **Note:** The `android/` and `ios/` directories are **generated by Capacitor**
> and are listed in `.gitignore`. They are created during the build process
> via `npx cap add android` and `npx cap sync android`.
...
1. Installs JDK 17, Android SDK (API 35), Gradle, Node.js 22
2. Installs npm dependencies (including Capacitor CLI and plugins)
3. Builds the web assets with Vite
4. Runs `npx cap add android` to generate the native Android project
5. Runs `npx cap sync android` to copy web assets and configure plugins
6. Patches Android config for local network cleartext traffic
7. Runs Gradle to build the debug APK
8. Extracts the APK to your `output/` directory
...
# 3. Generate Android project (first time only)
npx cap add android
